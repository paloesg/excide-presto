    ...
    module FrontendHelperDecorator
      ...
      def taxons_tree(root_taxon, current_taxon, max_level = 1)
        ...
        taxons = root_taxon.children.map do |taxon|
          ...
          sub_taxons = safe_join(taxon.children.map { |t| '<li class="col-md-6 p-b-10">' + link_to(t.name, seo_url(t)) + '</li>' })
        ...
        if services.present?
          static_pages = services.map do |service|
            ...
            sub_taxons = safe_join(service.children.map { |t| '<li class="col-md-6 p-b-10">' + link_to(t.name, seo_url(t)) + '</li>' })
      ...
      def link_to_cart(text = nil)
        ...
        if simple_current_order.nil? || simple_current_order.item_count.zero?
          text = "<i class='ti-bag'></i>"
          css_class = 'empty'
        else
          text = "<i class='ti-bag'></i> <span class='label number' id='total-items'>#{simple_current_order.item_count}</span>"
          css_class = 'full'
        end

        link_to text.html_safe, spree.cart_path, class: "cart cart-info #{css_class}", onclick: "Turbolinks.clearCache();"
      end
...
::Spree::FrontendHelper.prepend Presto::Spree::FrontendHelperDecorator